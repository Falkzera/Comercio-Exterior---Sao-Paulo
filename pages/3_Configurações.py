...
from src.utils.persistir_dados_bd import cadastrar_usuario, atualizar_usuario, gerar_hash_senha
from src.utils.recuperar_dados_bd import carregar_usuarios, carregar_usuario_id, email_existe
from src.utils.autenticacao import exigir_login, usuario_atual
...
st.sidebar.markdown("<h3 style='margin-bottom:0'>⚙️ Configurações</h3>", unsafe_allow_html=True)

modo = st.sidebar.radio("Ações", opcoes, key="settings_mode")
...
if modo == "Listar Usuários":
    ...
    for _, row in usuarios_df.iterrows():
        ...
        if not st.session_state[edit_key]:
            ...
            if perfil == "admin": # somente admin pode editar dados de usuários
                with col[5]:
                    if st.button("✏️ Editar", key=f"btn_edit_{uid}"):
    ...
    with st.expander("📋 Visualizar tabela completa (somente leitura)"):
        st.dataframe(
            usuarios_df[["idUsuario", "nome", "email", "perfil", "ativo", "criado_em", "ultima_modificacao", "ultimo_login"]])
